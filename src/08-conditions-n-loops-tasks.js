...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.left > rect2.left + rect2.width || rect2.left > rect1.width) return false;
  if (rect1.top > rect2.top + rect2.height || rect2.top > rect1.top + rect1.height) return false;
  return true;
}
...
function isCreditCardNumber(ccn) {
  const arrDigit = ccn.toString().split('');
  const isEvenCcn = !(arrDigit.length % 2); // четное - с первой цифры, нечетное - со второй

  let sumDigit = arrDigit.map((item, index) => {
    const doubleItem = item * 2;
    if (isEvenCcn) {
      if (!(index % 2)) return doubleItem > 9 ? doubleItem - 9 : doubleItem;
      return +item;
    }
  ...
  sumDigit = sumDigit.reduce((acc, item) => acc + item);
...
function getDigitalRoot(n) {
  function sum(num) {
    if (num < 9) return num;
    return (num % 10) + sum(Math.trunc(num / 10));
  }
...
function isBracketsBalanced(str) {
  ...
  for (const sym of str) {
    const symPos = config.indexOf(sym);

    if (!(symPos % 2)) stack.push(sym);
    else if (config.indexOf(stack[stack.length - 1]) === symPos - 1) stack.pop(sym);
    else return false;
  }
...
function getMatrixProduct(/* m1, m2 */) {
  // const sum = Array(m2[0].length).fill([]);
  // console.log(sum);
